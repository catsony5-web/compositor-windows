...
public static class DocumentFeatures
{
    ...
    public static void Rasterize(Layer layer)
    {
        ...
        layer.Kind = LayerKind.Raster; layer.Text = null; layer.Adjustment = null;
    }
    ...
    {
        spec.Validate(); return new() { Name = name ?? spec.Kind.ToString(), Kind = LayerKind.Adjustment, Adjustment = spec.Snapshot(), Pixels = new Raster(doc.Width, doc.Height) };
    }
    ...
    {
        spec.Validate(); var result = source.Clone();

...
{
    public LayerRow(Layer layer, bool selected, Action select, Action<bool> setVisible, Action toggleLock)
    {
        ...
        content.Children.Add(new Image { Source = layer.Pixels.Bitmap(), Width = 40, Height = 38, Stretch = Stretch.Uniform, Margin = new Thickness(3) });
        ...
        labels.Children.Add(new TextBlock { Text = $"{layer.Blend} · {layer.Opacity * 100:0}%{(layer.Mask != null ? " · 마스크" : "")}", Foreground = Theme.Muted, FontSize = 9, Margin = new Thickness(7, 0, 0, 0) });

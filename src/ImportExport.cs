...
public static class ImportExport
{
    ...
    public static void Export(Document document, string path, int quality = 95)
    {
        ...
        ProjectStore.AtomicWrite(path, stream => Write(raster, stream, Path.GetExtension(path), quality));
    }
    ...
    public static ExportPreview PreviewExport(Document document, string format, int quality = 95, int maxSide = 720)
    {
        ...
        Write(Imaging.Render(document), encoded, format, quality);
    ...
    public static void Write(Raster raster, Stream stream, string format, int quality = 95)
    {
        ...
        encoder.Frames.Add(BitmapFrame.Create(output.Bitmap()));

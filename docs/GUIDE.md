...
**0.2.0-preview.25 · Windows 10 2004 이상 / Windows 11 x64 · 독립 이미지 편집기**

**도면 레이어:** 도면을 열면 파일 하나로 접어서 표시합니다. 화살표를 펼치면 같은 CAD 레이어의 객체가 함께 보이며, 캔버스에서는 선·문자를 개별 선택합니다. 오른쪽의 **도면 레이어 / 포토샵 레이어**로 목록을 전환하세요.

**드래그로 객체 선택:** 이동 도구 **V**에서 빈 곳을 드래그합니다. **왼쪽 → 오른쪽** 파란 상자는 완전히 들어온 객체만, **오른쪽 → 왼쪽** 초록 상자는 일부만 닿은 객체도 선택합니다. Shift는 추가, Alt는 빼기입니다. 사진의 픽셀 선택은 기존 사각 선택 도구 **M**을 사용합니다.

**대지 편집:** **Shift+O** 또는 왼쪽 대지 아이콘을 누릅니다. 빈 곳을 드래그하면 대지가 생기고, 대지 안을 드래그하면 이동하며, 모서리 핸들로 크기를 바꿉니다. 기존 대지 안에 새로 만들 때는 Alt+드래그하세요. 속성에서 이름·위치·크기를 입력한 후 **변경 적용**, 편집을 마치면 **완료**를 누릅니다. 대지 수정은 안의 객체를 이동하거나 확대하지 않습니다. 내보내기 창에서 저장할 대지를 선택할 수 있습니다.

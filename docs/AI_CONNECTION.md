...
**Preview 19 로컬 검증 후보**의 연결 안내입니다. 공개 다운로드에 이 기능이 배포되었다는 뜻은 아닙니다.
...
`open_document`는 기존 가져오기 엔진을 사용합니다. PDF/PDF 호환 AI는 기본값인 **첫 페이지·150 DPI**, PSD/PSB는 합성 이미지, DWG/DXF는 기본 **긴 변 2,400px의 이미지**로 가져옵니다. 페이지·레이어 분리 같은 세부 가져오기 옵션은 현재 자동화 명령에 없습니다. Adobe·AutoCAD 프로그램을 조작하거나 원본의 모든 문자·벡터·CAD 속성을 편집하는 기능은 아닙니다. [파일별 보존 범위](FILE_COMPATIBILITY.md)
